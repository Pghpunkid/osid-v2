<?php
	...
	$WriteStatus = shell_exec("cat /etc/osid/www/public_html/system/status.info");

	//declare the line variable
	$line = "";

	//open the progress info file and set cursor to -1
	$f = fopen('/etc/osid/www/public_html/system/progress.info', 'r');
	$cursor = -1;
	...
	if ($PercentCompleted == 100 && $WriteStatus == 2) {
		shell_exec("echo \"0\" > /etc/osid/www/public_html/system/status.info");
	}
